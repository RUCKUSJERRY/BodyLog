...
public class Util {
	...
	// 날짜 형태의 데이터를 String으로 변경
	
	// String형태의 데이터를 날짜 형태로 변경
...
private String todates;
	...
	public void setTodates(String mdate) {
		...
		String temp = mdate.substring(0, 4)+"-"
				    + mdate.substring(4, 6)+"-"
				    + mdate.substring(6, 8)+" T"
				    + mdate.substring(8, 10)+":"
				    + mdate.substring(10, 12)+":"
					+ mdate.substring(12);
		
		// yyyy년MM월dd일 HH시mm분 형태로 날짜/시간 패턴을 설정하여 sdf 변수에 대입
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd THH:mm:ss");
	...
	public String getTostrings(Date date) {
		...
		SimpleDateFormat transFormat = new SimpleDateFormat("yyyyMMdd THH:mm:ss");
